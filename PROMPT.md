...
> resulting project is publishable as open source. The maintainer's own instance (`<ha-host>`) is
...
- The reference host is a low-power machine running **rootless Podman with Quadlet**, reachable via
  `ssh <ha-host>`, already running HA plus a few sibling containers. Study its Quadlet convention as the model
...
- **SSH:** allowed ONLY to `ssh <ha-host>` (the maintainer's reference host; a user with sudo nopasswd) — for
